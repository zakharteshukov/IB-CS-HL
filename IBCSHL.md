...
- `units/unitN/weekN/` → per‑week overview (N.0.md) and lesson notes (N.1.md, N.2.md, …)
- `units/unitN/.html/weekN/` → original lesson HTML (backup)
- `resources/` → IBO guides, attachments, and reference materials
- `topics/` → Topic-based notes organized by IB syllabus (A.1.1, A.1.2, etc.)
- `documents/` → Weekly PDF documents and handouts
- `drawings/` → Excalidraw diagrams and visual notes

**See `README.md` for complete folder structure and navigation guide.**
...
### Converting content
- "Convert `units/unitN/.html/weekX/X.Y.html` into Notion‑ready Markdown. Preserve headings (H1/H2/H3), tables, lists. Replace iframes with plain URLs."
- "Fix image/video embeds to standard Markdown; use plain YouTube links."

### Studying & revision
- "Summarize `units/unitN/weekX/X.Y.md` into key points and exam‑style Q&A."
...
### Linking & navigation
- "Update `README.md` or `IBCSHL.md` link tree to include week N lessons that were added today."
...
- Do not move files—use existing structure (`units/unitN/weekN/`).
- When creating new notes, prefer `N.Z.md` naming (e.g., `6.5.md`).
- For Obsidian, keep attachments in `resources/.attachments/` and use relative links.
- Follow the organized folder structure: `units/`, `resources/`, `topics/`, `documents/`, `drawings/`.
